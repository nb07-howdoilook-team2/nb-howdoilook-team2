...
const createCuration = async ({ styleId, ...curationData }) => {
  ...
  // 큐레이션 생성시 스타일 평균 점수 더하기
  const curations = await curationRepository.findByStyleId(styleId);
  const ratings = calculateRatings(curations);

  await styleRepository.updateStyleRatings(styleId, {
    ...ratings,
...
const updateCuration = async (curationId, inputPassword, updateData) => {
  ...
  // 큐레이션 수정시 스타일 평균 점수 다시 계산 및 업데이트
  const styleId = curationInfo.styleId;
...
const deleteCuration = async (curationId, inputPassword) => {
  ...
  // 큐레이션 삭제시 스타일 평균 점수 다시 계산 및 업데이트
  const styleId = curationInfo.styleId;
  const curations = await curationRepository.findByStyleId(styleId);
  const ratings = calculateRatings(curations);
